...
public class QryParser {
  ...
  private static Qry createOperator (String operatorName) {
    ...
    String operatorNameFinal = operatorNameLowerCase;
    int distance = 0;
    if (operatorNameLowerCase.contains("/")) {
      String[] s = operatorNameLowerCase.split("/");
      operatorNameFinal = s[0];
      distance = Integer.parseInt(s[1]);
    }

    //  Create the query operator.
    switch (operatorNameFinal) {
      case "#or":
  ...
  private static Qry parseString (String queryString)
    ...
    while (queryString.length() > 0) {
      ...
      double weight = 0.0;
      boolean isWAndOperator = false;
      boolean isWSumOperator = false;
      ...
      //  Handle the argument (which could be a subquery).
      ...
      if (queryString.charAt(0) != '#') {
        ...
        if ((isWAndOperator || isWSumOperator)) {
	...
	//  Handle different weights
        if (isWAndOperator) {
          ((QrySopWAnd)queryTree).addWeight(weight);
        } else if (isWSumOperator) {
          ((QrySopWSum)queryTree).addWeight(weight);
        }
